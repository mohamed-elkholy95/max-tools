...
export function generateLoremIpsum({
  ...
  const safeSentenceCount = Math.max(1, Math.floor(sentencePerParagraph));

  if (safeParagraphCount === 0) {
    return '';
  }

  const paragraphs = Array.from({ length: safeParagraphCount }).map(() =>
    Array.from({ length: safeSentenceCount }).map(() => generateSentence(wordCount)),
  );
