...
function toSelectOptions(options: any[]) {
  return options.flatMap((opt) => {
    const anyOpt = opt as any;
    if (anyOpt.options && Array.isArray(anyOpt.options)) {
      return anyOpt.options.map((o: any) => String(o.value ?? o.label));
    }
    return [String(anyOpt.value ?? anyOpt.label)];
  });
}
</script>
    ...
    <div v-for="{ name, elements } of sections" :key="name" style="margin-bottom: 15px">
      ...
      <n-input-group v-for="{ key, type, label, placeholder, ...element } of elements" :key="key">
        ...
        <c-select
          ...
          w-full
          :placeholder="placeholder"
          :options="toSelectOptions((element as OGSchemaTypeElementSelect).options)"
        />
      </n-input-group>
    </div>
  </div>
